...
void C4() {
  ...
  auto find_hash = [&](std::string base, uint32_t start, uint32_t increment) {
    ...
    while (run) {
      ...
      bool check_zeros =
          n.as_bytes[0] == 0 && n.as_bytes[1] == 0 && n.as_bytes[2] == 0;
...

  {
    ...
    await_result.wait(lock, [&]() { return run == false; });
  }
...
void C6() {
  enum class Instruction { NONE, TOGGLE, ON, OFF };
  struct Entry {
...
void C7() {
  ...
  enum class Operation { NONE, AND, OR, NOT, RSHIFT, LSHIFT };
  struct BinaryOp {
...
void C11() {
  ...
  while (!validPassword) {
    ...
    validPassword =
        CheckILO(digits) && CheckDoublePair(digits) && CheckStraight(digits);
  }
...
void C14() {
  ...
  std::sort(all_runners.begin(), all_runners.end(),
            [&](const auto &lhs, const auto &rhs) {
              return lhs.CalculateDistance(race_time) >
                     rhs.CalculateDistance(race_time);
            });
  ...
  auto it =
      std::max_element(score_accumulated.begin(), score_accumulated.end());
    ...
    for (auto &selected_armor : armor_shop) {
      for (auto &acc1 : ring_shop)
        for (auto &acc2 : ring_shop) {
          ...
          Stats test = player;
          int gold_spent = selected_weapon.first + selected_armor.first +
                           std::get<0>(acc1) + std::get<0>(acc2);
          test.armor =
              selected_armor.second + std::get<1>(acc1) + std::get<1>(acc2);
          test.damage =
              selected_weapon.second + std::get<2>(acc1) + std::get<2>(acc2);
...
void C22() {
  ...
  enum class State { RUNNING, VICTORY, DEFEAT, INVALID };
...
void C23() {
  struct VirtualMachine {
    ...
    void run(std::vector<std::string> &program) {
      ...
      } while (instruction_ptr < (int32_t)program.size() &&
               instruction_ptr >= 0);
    }
